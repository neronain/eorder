...
function CompileTemplate($path,$exception=NULL){
	//echo "{$path}<br />";
	...
	while ($file = readdir($dir_handle)) 
	{
		...
		if($is_repeat){
			...
			$rwcode = preg_replace('/> *([©\*\+‘’"a-z0-9 \(\)%\[\]\{\}&\.\/,\t_\-\$;!:=@\|\?#]+?) *(<|$|\n)/i',"><?=__('$1')?>$2",$rwcode);
			//replace empty __(' ')
			$rwcode = preg_replace("/<\?=__\('( |&nbsp;|\.|\t)+'\)\?>/","$1",$rwcode);
			
			$rwcode = preg_replace('/(placeholder|label|alt) *= *"(.*?)"/is','$1="<?=__("$2")?>"',$rwcode);
...
CompileTemplate("../../app_hexa",array("_cron","lang","_include","_internalapp","_lib","_model","_plugins","_skeleton","_uncat"));
